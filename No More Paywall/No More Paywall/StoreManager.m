   ...
   // MyModel *model = nil;
    ...
    if ((response.products).count > 0)
    {
       // model = [[MyModel alloc] initWithName:@"AVAILABLE PRODUCTS" elements:response.products];
        [self.productRequestResponse addObject:products.firstObject];
    ...
    SKProduct *book = [products objectAtIndex:0];
    
    NSNumberFormatter *numberFormatter = [[NSNumberFormatter alloc] init];
    [numberFormatter setFormatterBehavior:NSNumberFormatterBehavior10_4];
    [numberFormatter setNumberStyle:NSNumberFormatterCurrencyStyle];
    [numberFormatter setLocale:book.priceLocale];
    NSString *currencyString = [numberFormatter stringFromNumber:book.price];

    NSLocale* storeLocale = book.priceLocale;
    NSString *storeCountry = (NSString*)CFLocaleGetValue((CFLocaleRef)storeLocale, kCFLocaleCountryCode);
    
    NSString *price = [NSString stringWithFormat:@"%@%@", [book.priceLocale objectForKey:NSLocaleCurrencySymbol], book.price];
